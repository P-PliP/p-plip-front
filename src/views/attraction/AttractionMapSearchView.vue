<template>
  <div class="map-search-view">
    <!-- Map (Full Screen) -->
    <div class="map-area">
      <KakaoMap ref="mapRef" :lat="center.lat" :lng="center.lng" width="100%" height="100%"
        @onLoadKakaoMap="onLoadKakaoMap">
        <!-- Existing Plan Markers -->
        <KakaoMapCustomOverlay v-for="(item, index) in existingPlanItems" :key="'existing-' + index"
          :lat="Number(item.latitude)" :lng="Number(item.longitude)" :yAnchor="1">
        ...
        <!-- Search Result Markers -->
        <!-- Search Result Markers -->
        <KakaoMapCustomOverlay v-for="(place, index) in results" :key="getPlaceId(place) || index"
          :lat="Number(place.latitude)" :lng="Number(place.longitude)" :yAnchor="1" :zIndex="10">
        ...
        <KakaoMapMarker v-if="userLocation" :lat="userLocation.lat" :lng="userLocation.lng" :image="{
    ...
    <div class="floating-search-container">
      ...
      <!-- Results Card (Separated) -->
      <div class="results-container" v-if="hasSearched">
        <!-- Results Header (Always Visible, Toggle Trigger) -->
        <div class="results-header" @click="toggleList">
          <span class="results-title">검색 결과 {{ results.length }}개</span>
          <button class="toggle-btn" :class="{ collapsed: !isListExpanded }" title="목록 접기/펴기">
            <svg class="toggle-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#555"
        ...
        <!-- Collapsible Body -->
        <div class="results-collapse-container" :class="{ collapsed: !isListExpanded }">
          <!-- Search Results List -->
          <div class="search-results-card"> <!-- Note: reused class for internal list styling -->
            ...
            <div v-else class="results-list" @scroll="onListScroll">
              <div v-for="place in results" :key="getPlaceId(place)" class="place-item"
    ...
    <!-- Load More Button (Floating Bottom Center) -->
    ...
    <Teleport to="body">
      <div v-if="selectedPlace" class="modal-overlay" @click.self="selectedPlace = null">
        <div class="place-card-wrapper">
          <PlaceDetailSheet :place="selectedPlace" :hide-actions="true" @close="selectedPlace = null">
            <template #footer>
...
/* ... (Keep existing styles until search-header-card) ... */
...
.floating-search-container {
  ...
  gap: 12px;
  /* Increased gap between controls and results */
...
.results-collapse-container.collapsed {
  max-height: 0;
  opacity: 0;
}

/* Internal Results List Styling */
.search-results-card {
  /* Simplified as it's now inside the container */
  background: transparent;
  box-shadow: none;
  border-radius: 0;
  max-height: 400px;
